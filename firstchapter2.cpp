...
firstchapter2::firstchapter2(QWidget *parent)
   : QWidget(parent)
{
        ...
        QPalette palette;
        palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/firstback2.png").scaled(this->size())));
        ...
        connect(timer, &QTimer::timeout, this, &firstchapter2::timerUpdate);
        timer->start(10);
}
firstchapter2::~firstchapter2()
{

}
void firstchapter2::paintEvent(QPaintEvent *)
{
...
void firstchapter2::keyPressEvent(QKeyEvent *event)
{
    int key1 = event->key();
    switch(key1)
    {
...
void firstchapter2::keyReleaseEvent(QKeyEvent *event)
{
    int key2 = event->key();
    switch(key2)
    {
...
void firstchapter2::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void firstchapter2::ecexist() //菜鸡死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
        for (int j = 0;j < 100; j++)
...
void firstchapter2::myexist()//自己死不死
{
...
void firstchapter2::efexist() //杂鱼死不死
{
    for (int i = 0; i < 200; i++)
    {
...
void firstchapter2::efbulletexist()//杂鱼的子弹死不死
{
    for (int i = 0; i < 200; i++)
    {
        for (int j = 0; j < 3; j++)
...
void firstchapter2::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
...
void firstchapter2::firstmapexist()
{
    if (my.myflyingy < 500)
        firstmap->load("../project-final/window.png");
}

void firstchapter2::returnslot()
{
    this->hide();
    player->stop();
...
void firstchapter2::pauseslot()
{
    timer->stop();
    player->pause();
}

void firstchapter2::continueslot()
{
    timer->start(10);
    player->play();
}

void firstchapter2::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
...
void firstchapter2::restartslot()
{
...
void firstchapter2::nostartslot()
{
...
void firstchapter2::gameclear()
{
    secondchapter2 *secondguan = new secondchapter2();
    player->stop();
    this->~firstchapter2();
    secondguan->show();
}
