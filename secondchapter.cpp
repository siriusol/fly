...
secondchapter::secondchapter(QWidget *parent)
   : QWidget(parent)
{
        ...
        QPalette palette;
        palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/secondback.png").scaled(this->size())));
        ...
        connect(timer, &QTimer::timeout, this, &secondchapter::timerUpdate);
        timer->start(10);
}
...
void secondchapter::paintEvent(QPaintEvent *)
{
...
void secondchapter::keyPressEvent(QKeyEvent *event)
{
    int key1 = event->key();
    switch(key1)
    {
...
void secondchapter::keyReleaseEvent(QKeyEvent *event)
{
    int key2 = event->key();
    switch(key2)
    {
...
void secondchapter::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void secondchapter::ecexist() //菜鸡死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
        for (int j = 0;j < 100; j++)
...
void secondchapter::myexist()//自己死不死
{
...
void secondchapter::efexist() //杂鱼死不死
{
    for (int i = 0; i < 200; i++)
    {
...
void secondchapter::efbulletexist()//杂鱼的子弹死不死
{
    for (int i = 0; i < 200; i++)
    {
        for (int j = 0; j < 3; j++)
...
void secondchapter::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
...
void secondchapter::secondmapexist()
{
    if (my.myflyingy < 500)
        secondmap->load("../project-final/window.png");
}

void secondchapter::returnslot()
{
    this->hide();
    player->stop();
...
void secondchapter::pauseslot()
{
    timer->stop();
    player->pause();
}

void secondchapter::continueslot()
{
    timer->start(10);
    player->play();
}

void secondchapter::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
...
void secondchapter::restartslot()
{
    secondchapter *rebootone = new secondchapter();
    this->hide();
    rebootone->show();
}

void secondchapter::nostartslot()
{
...
void secondchapter::gameclear()
{
    player->stop();
    thirdchapter *thirdguan = new thirdchapter();
    this->~secondchapter();
    thirdguan->show();
}
