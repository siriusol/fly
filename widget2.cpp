   ...
   : QWidget(parent)
{
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/fourback2.png").scaled(this->size())));
    ...
    for (int i = 0; i < 1000; i++)//boss1的1000个子弹初始布局
    {
        ...
        boss1->boss1bullet1[i]->ebulletmap.load("../project-final/bossbullet42.png");
        boss1->boss1bullet2[i]->ebulletmap.load("../project-final/bossbullet52.png");
        boss1->boss1bullet3[i]->ebulletmap.load("../project-final/bossbullet62.png");
        if (i % 19 < 10)
    ...
    connect(timer, &QTimer::timeout, this, &Widget2::timerUpdate);
    timer->start(10);
...
void Widget2::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void Widget2::myexistboss()//自己死不死boss
{
    if (!boss1pd)
        return;
    for (int j = 0; j < 100; j++)
...
void Widget2::efbulletexist()//杂鱼的子弹死不死
{
    if (boss1pd)
        return;
    for (int i = 0; i < 200; i++)
...
void Widget2::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
    if (boss1pd)
        return;
...
void Widget2::boss1bulletexist()//boss的子弹类型1和类型2死不死
{
    if (!boss1pd)
        return;
    for (int k = 0; k < my.bulletcount;k++)
...
void Widget2::fourmapexist()
{
    if (my.myflyingy < 500)
        fourmap->load("../project-final/window.png");
}

void Widget2::returnslot()
{
    this->hide();
    player->stop();
...
void Widget2::pauseslot()
{
    timer->stop();
    player->pause();
}

void Widget2::continueslot()
{
    timer->start(10);
    player->play();
}

void Widget2::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
