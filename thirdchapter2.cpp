...
thirdchapter2::thirdchapter2(QWidget *parent)
   : QWidget(parent)
{
    this->resize(1080,720);
    this->setWindowTitle("第三关");
    this->setWindowIcon(QIcon("../project-final/icon.png"));
    setWindowFlags(windowFlags()&~Qt::WindowMaximizeButtonHint);

    player = new QMediaPlayer();
    player->setMedia(QUrl::fromLocalFile("../project-final/3.mp3"));
    player->setVolume(20);
    player->play();
    thirdmap = new QPixmap("../project-final/thirdmap.png");
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/thirdback2.png").scaled(this->size())));
    ...
    for(int i = 0; i < 200; i++)//200个杂鱼及每个5个子弹布局
    {
         for (int j = 0; j < 5; j++)
         {
             fish[i]->eb[j] = new enemybullet2(fish[i]->fx + 20,fish[i]->fy + 80 + j * 80);
         }
    }
    ...
    {
        boss1->boss1bullet1[i] = new enemybullet2(boss1->bossx,boss1->bossy);
        boss1->boss1bullet2[i] = new enemybullet2(boss1->bossx,boss1->bossy);
        boss1->boss1bullet3[i] = new enemybullet2(boss1->bossx,boss1->bossy);
        boss1->boss1bullet1[i]->ebulletmap.load("../project-final/bossbullet12.png");
        boss1->boss1bullet2[i]->ebulletmap.load("../project-final/bossbullet22.png");
        boss1->boss1bullet3[i]->ebulletmap.load("../project-final/bossbullet32.png");
        if (i % 19 < 10)
    ...
    connect(timer, &QTimer::timeout, this, &thirdchapter2::timerUpdate);
    timer->start(10);

}

thirdchapter2::~thirdchapter2()
{

}

void thirdchapter2::paintEvent(QPaintEvent *)
{
    fen->setText(QString::number(killfen));
    lifenumlabel->setText(QString::number(lifenum));

    painter = new QPainter(this);
    painter->drawPixmap(0,300,1080,226,*thirdmap);
    if (lifenum == 5)
    {
    ...
    if (killfen >= 4500 && !boss1pd)
        painter->drawPixmap(0,300,1080,226,*warning);
    if (killfen >= 5000)//boss出现的判断
    {
        painter->end();
        boss1pd = true;
...
void thirdchapter2::keyPressEvent(QKeyEvent *event)
{
    int key1 = event->key();
    switch(key1)
    {
...
void thirdchapter2::keyReleaseEvent(QKeyEvent *event)
{
    int key2 = event->key();
    switch(key2)
    {
...
void thirdchapter2::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void thirdchapter2::ecexist() //菜鸡死不死
{
    if (boss1pd)
        return;
    for(int i = 0;i < my.bulletcount;i++)
...
void thirdchapter2::myexist()//自己死不死
{
...
void thirdchapter2::myexistboss()//自己死不死boss
{
    if (!boss1pd)
        return;
    for (int j = 0; j < 100; j++)
...
void thirdchapter2::efexist() //杂鱼死不死
{
    if (boss1pd)
        return;
    for (int i = 0; i < 200; i++)
...
void thirdchapter2::efbulletexist()//杂鱼的子弹死不死
{
    if (boss1pd)
        return;
    for (int i = 0; i < 200; i++)
...
void thirdchapter2::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
    if (boss1pd)
        return;
...
void thirdchapter2::boss1bulletexist()//boss的子弹类型1和类型2死不死
{
    if (!boss1pd)
        return;
    for (int k = 0; k < my.bulletcount;k++)
...
void thirdchapter2::thirdmapexist()
{
    if (my.myflyingy < 500)
        thirdmap->load("../project-final/window.png");
}

void thirdchapter2::returnslot()
{
    this->hide();
    player->stop();
...
void thirdchapter2::pauseslot()
{
    timer->stop();
    player->pause();
}

void thirdchapter2::continueslot()
{
    timer->start(10);
    player->play();
}

void thirdchapter2::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
...
void thirdchapter2::restartslot()
{
    firstchapter2 *rebootone = new firstchapter2();
    this->hide();
    rebootone->show();
}

void thirdchapter2::nostartslot()
{
...
void thirdchapter2::gameclear()
{
    player->stop();
    Widget2 *fourguan = new Widget2();
    fourguan->show();
    this->hide();
}
