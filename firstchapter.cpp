...
firstchapter::firstchapter(QWidget *parent)
   : QWidget(parent)
{
        ...
        player->setMedia(QUrl::fromLocalFile("../project-final/1.mp3"));
        player->setVolume(20);
        player->play();
        firstmap = new QPixmap("../project-final/firstmap.png");
        warning = new QPixmap("../project-final/warning.png");
        gameovermap = new QPixmap("../project-final/window.png");

        QPalette palette;
        palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/firstback.png").scaled(this->size())));
        ...
        for(int i = 0; i < 200; i++)//200个杂鱼及每个5个子弹布局
        {
             for (int j = 0; j < 5; j++)
             {
                 fish[i]->eb[j] = new enemybullet(fish[i]->fx + 20,fish[i]->fy + 80 + j * 100);
             }
        }
        ...
        connect(timer, &QTimer::timeout, this, &firstchapter::timerUpdate);
        timer->start(10);
}
...
void firstchapter::paintEvent(QPaintEvent *)
{
    fen->setText(QString::number(killfen));
    lifenumlabel->setText(QString::number(lifenum));

    painter = new QPainter(this);
    painter->drawPixmap(0,300,1080,226,*firstmap);
    if (lifenum == 5)
        {
...
void firstchapter::keyPressEvent(QKeyEvent *event)
{
    int key1 = event->key();
    switch(key1)
    {
...
void firstchapter::keyReleaseEvent(QKeyEvent *event)
{
    int key2 = event->key();
    switch(key2)
    {
...
void firstchapter::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void firstchapter::ecexist() //菜鸡死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
        for (int j = 0;j < 100; j++)
...
void firstchapter::myexist()//自己死不死
{
...
void firstchapter::efexist() //杂鱼死不死
{
    for (int i = 0; i < 200; i++)
    {
...
void firstchapter::efbulletexist()//杂鱼的子弹死不死
{
    for (int i = 0; i < 200; i++)
    {
        for (int j = 0; j < 3; j++)
...
void firstchapter::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
    ...
    chickenlabelnum++;
    //QTimer::singleShot(2500,this,SLOT(labelhide()));
}

void firstchapter::firstmapexist()
{
    if (my.myflyingy < 500)
        firstmap->load("../project-final/window.png");
}

void firstchapter::returnslot()
{
    this->hide();
    player->stop();
...
void firstchapter::pauseslot()
{
    timer->stop();
    player->pause();
}

void firstchapter::continueslot()
{
    timer->start(10);
    player->play();
}

void firstchapter::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
...
void firstchapter::gameclear()
{
    secondchapter *secondguan = new secondchapter();
    player->stop();
    this->~firstchapter();
    secondguan->show();
}
