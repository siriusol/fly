...
thirdchapter::thirdchapter(QWidget *parent)
   : QWidget(parent)
{
    ...
    QPalette palette;
    palette.setBrush(QPalette::Background,QBrush(QPixmap("../project-final/thirdback.png").scaled(this->size())));
    ...
    boss1 = new boss();
    //boss1->bossmap =  QPixmap("../project-final/boss.png");
    for (int i = 0; i < 1000; i++)//boss1的1000个子弹初始布局
    {
        ...
        boss1->boss1bullet1[i]->ebulletmap.load("../project-final/bossbullet1.png");
        boss1->boss1bullet2[i]->ebulletmap.load("../project-final/bossbullet2.png");
        boss1->boss1bullet3[i]->ebulletmap.load("../project-final/bossbullet3.png");
        if (i % 19 < 10)
    ...
    connect(timer, &QTimer::timeout, this, &thirdchapter::timerUpdate);
    timer->start(10);
...
void thirdchapter::paintEvent(QPaintEvent *)
{
...
void thirdchapter::keyPressEvent(QKeyEvent *event)
{
    int key1 = event->key();
    switch(key1)
    {
...
void thirdchapter::keyReleaseEvent(QKeyEvent *event)
{
    int key2 = event->key();
    switch(key2)
    {
...
void thirdchapter::bulletexist() //自己的子弹死不死
{
    for(int i = 0;i < my.bulletcount;i++)
    {
...
void thirdchapter::ecexist() //菜鸡死不死
{
    if (boss1pd)
        return;
    for(int i = 0;i < my.bulletcount;i++)
...
void thirdchapter::myexist()//自己死不死
{
...
void thirdchapter::myexistboss()//自己死不死boss
{
    if (!boss1pd)
        return;
    for (int j = 0; j < 100; j++)
...
void thirdchapter::efexist() //杂鱼死不死
{
    if (boss1pd)
        return;
    for (int i = 0; i < 200; i++)
...
void thirdchapter::efbulletexist()//杂鱼的子弹死不死
{
    if (boss1pd)
        return;
    for (int i = 0; i < 200; i++)
...
void thirdchapter::explode(int x,int y)//显示"爆",菜鸡的死亡效果
{
    if (boss1pd)
        return;
...
void thirdchapter::boss1bulletexist()//boss的子弹类型1和类型2死不死
{
    if (!boss1pd)
        return;
    for (int k = 0; k < my.bulletcount;k++)
...
void thirdchapter::thirdmapexist()
{
    if (my.myflyingy < 500)
        thirdmap->load("../project-final/window.png");
}

void thirdchapter::returnslot()
{
    this->hide();
    player->stop();
...
void thirdchapter::pauseslot()
{
    timer->stop();
    player->pause();
}

void thirdchapter::continueslot()
{
    timer->start(10);
    player->play();
}

void thirdchapter::gameover()
{
    if (lifenum < 1)
    {
        timer->stop();
...
void thirdchapter::gameclear()
{
    player->stop();
    Widget *fourguan = new Widget();
    fourguan->show();
    this->hide();
}
